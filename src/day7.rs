...
pub(crate) fn run(input: &String) -> Result<()> {
    ...
    for dep in dependencies {
        dependency_map.entry(dep.letter).or_default().insert(dep.depends_on);
...
fn get_next_steps(dependency_map: &HashMap<char, HashSet<char>>, steps_taken: &HashSet<char>, in_progress: Option<&HashSet<char>>) -> Vec<char> {
...
impl ElfPool {
    fn new(capacity: usize) -> Self {
        ElfPool { elves: vec![ElfStatus::Idle; capacity] }
    }
    ...
    fn run(&mut self) -> Vec<char> {
        ...
        for i in 0..self.elves.len() {
            ...
            match self.elves[i] {
                ElfStatus::Idle => {},
                ElfStatus::Working { step, ref mut remaining } => {
                    *remaining -= 1;
                    if *remaining == 0 {
                        println!("{} is idle", i);
                        is_idle = true;
...
impl FromStr for Dependency {
    ...
    fn from_str(s: &str) -> Result<Dependency> {
        ...
        if let Some(captures) = REGEX.captures(s) {
            return Ok(Dependency { letter: captures[2].as_bytes()[0] as char, depends_on: captures[1].as_bytes()[0] as char })
        }
